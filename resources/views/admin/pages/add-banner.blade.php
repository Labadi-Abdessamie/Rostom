...
@section('content')
        ...
        <div class="ed-main">
            ...
            <div class="ed-right">
                ...
                <div class="ed-panels">
                    ...
                    {{-- PROPERTIES TAB (Banner Metadata + Element Props) --}}
    ...
    <form id="bannerForm" action="{{ route('admin.store_banner') }}" method="POST" enctype="multipart/form-data" style="display:none;">
        @csrf
        ...
        <input type="hidden" name="page" id="formPage" value="{{ $banner->page ?: 'home' }}">
        <input type="hidden" name="position" id="formPosition" value="{{ $banner->position ?: 'hero' }}">
        <input type="hidden" name="priority" id="formPriority" value="{{ $banner->priority ?: 1 }}">
...
@section('scripts')
    ...
    (function() {
        ...
        // Preview: only available after the banner has been saved (has an ID)
        const bannerId = {{ $banner->id ? $banner->id : 'null' }};
        if (bannerId) {
            $('#previewToggle').addEventListener('click', () => {
                saveDesign().then(() => {
                    window.open('/admin/banner/' + bannerId + '/preview', '_blank');
                });
            });
        } else {
            $('#previewToggle').addEventListener('click', () => {
                alert('Please save the banner first before previewing.');
            });
            $('#previewToggle').style.opacity = '0.5';
            $('#previewToggle').style.cursor = 'not-allowed';
        }
        ...
        function saveDesign() {
            return new Promise((resolve) => {
                ...
                // Set title from first text element or banner name input
                $('#formTitle').value = (firstText && firstText.content) ? firstText.content : $('#bannerName').value;
                // Set description: use subtitle text or a derived text, or leave blank
                const subtitle = design.elements.find(e => e.type === 'text' && e.name && e.name.toLowerCase().includes('subtitle'));
                $('#formDescription').value = (subtitle && subtitle.content) ? subtitle.content
                    : (firstText ? (firstText.content || '').slice(0, 140) : '');
                // Link: default to button link, or empty string (not '#', to pass URL validation)
                $('#formLink').value = (firstBtn && firstBtn.link) ? firstBtn.link : '';
                // Sync page/position/priority from sidebar metadata fields
                if ($('#metaPage'))    document.getElementById('formPage').value    = $('#metaPage').value;
                if ($('#metaPosition')) document.getElementById('formPosition').value = $('#metaPosition').value;
                if ($('#metaPriority')) document.getElementById('formPriority').value = $('#metaPriority').value;
