...
<section class="section">
    ...
    <div class="section-body">
                ...
                <div class="card mb-0">
                    <div class="card-body">
                        <ul class="nav nav-pills" id="orders-tab" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" id="all-orders-tab" data-bs-toggle="pill" href="#all-orders">All <span class="badge badge-white">{{ $orders->count() }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" id="pending-orders-tab" data-bs-toggle="pill" href="#pending-orders">Pending <span class="badge badge-primary">{{ $orders->where('status', 'pending')->count() }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" id="confirmed-orders-tab" data-bs-toggle="pill" href="#confirmed-orders">Confirmed <span class="badge badge-primary">{{ $orders->where('status', 'confirmed')->count() }}</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" id="completed-orders-tab" data-bs-toggle="pill" href="#completed-orders">Completed <span class="badge badge-primary">{{ $orders->where('status', 'completed')->count() }}</span></a>
                            </li>
                        </ul>
                    </div>
                </div>
        ...
        <div class="tab-content" id="orders-tabContent">
            @php
                $statuses = ['all' => $orders, 'pending' => $orders->where('status', 'pending'), 'confirmed' => $orders->where('status', 'confirmed'), 'completed' => $orders->where('status', 'completed')];
            @endphp

            @foreach ($statuses as $key => $statusOrders)
                <div class="tab-pane fade {{ $key === 'all' ? 'show active' : '' }}" id="{{ $key }}-orders" role="tabpanel">
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
